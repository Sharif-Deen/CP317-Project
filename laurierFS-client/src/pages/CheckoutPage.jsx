...
export default function CheckoutPage() {
  ...
  const [payment, setPayment] = useState({
    method: "credit", cardNumber: "", cardHolder: "", expiry: "", cvv: "",
  });
    ...
    <div className="checkout-page">
      ...
      <div className="checkout-content">
        ...
        <div className="checkout-steps">
          <div className={`checkout-step ${step >= 1 ? "active" : ""}`}>1. Shipping</div>
          <div className="checkout-step-divider" />
          <div className={`checkout-step ${step >= 2 ? "active" : ""}`}>2. Payment</div>
          <div className="checkout-step-divider" />
          <div className={`checkout-step ${step >= 3 ? "active" : ""}`}>3. Receipt</div>
        </div>
          ...
          <div className="checkout-form-section">
            ...
            {step === 1 && (
              <div className="checkout-card">
                ...
                <div className="form-group">
                  ...
                  <input type="text" placeholder="John Doe" value={shipping.fullName}
                    onChange={(e) => setShipping({ ...shipping, fullName: e.target.value })} />
                  {errors.fullName && <p className="error">{errors.fullName}</p>}
                </div>
                <div className="form-group">
                  <label>Delivery Address</label>
                  <input type="text" placeholder="123 Main St" value={shipping.address}
                ...
                <div className="form-row">
                  <div className="form-group">
                    <label>City</label>
                    <input type="text" placeholder="Waterloo" value={shipping.city}
                      onChange={(e) => setShipping({ ...shipping, city: e.target.value })} />
                    {errors.city && <p className="error">{errors.city}</p>}
                  </div>
                  <div className="form-group">
                    <label>Postal Code</label>
                    <input type="text" placeholder="N2L 3C5" value={shipping.postalCode}
                ...
                <div className="form-group">
                  ...
                  <input type="text" placeholder="519-123-4567" value={shipping.phone}
                    onChange={(e) => setShipping({ ...shipping, phone: e.target.value })} />
                  {errors.phone && <p className="error">{errors.phone}</p>}
                </div>
                <button className="checkout-btn" onClick={handleShippingNext}>Continue to Payment →</button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="checkout-card">
                ...
                <div className="payment-toggle">
                  <button className={`toggle-btn ${payment.method === "credit" ? "selected" : ""}`}
                    onClick={() => setPayment({ ...payment, method: "credit" })}>Credit Card</button>
                  <button className={`toggle-btn ${payment.method === "debit" ? "selected" : ""}`}
                    onClick={() => setPayment({ ...payment, method: "debit" })}>Debit Card</button>
                </div>
                <div className="form-group">
                  <label>Card Number</label>
                  <input type="text" placeholder="1234 5678 9012 3456" maxLength={16} value={payment.cardNumber}
                    onChange={(e) => setPayment({ ...payment, cardNumber: e.target.value })} />
                  {errors.cardNumber && <p className="error">{errors.cardNumber}</p>}
                </div>
                <div className="form-group">
                  <label>Card Holder Name</label>
                  <input type="text" placeholder="John Doe" value={payment.cardHolder}
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    <label>CVV</label>
                    <input type="password" placeholder="123" maxLength={3} value={payment.cvv}
            ...
            {step === 3 && (
              <div className="checkout-card receipt-card">
                ...
                <p className="receipt-info">{shipping.city}, {shipping.postalCode}</p>
                <p className="receipt-info">{shipping.phone}</p>
                <div className="receipt-divider" />
                <h4 className="receipt-section-title">Payment</h4>
                <p className="receipt-info">
                  {payment.method === "credit" ? "Credit Card" : "Debit Card"} ending in {payment.cardNumber.slice(-4)}
                </p>
                <div className="receipt-divider" />
            ...
            <div className="checkout-summary">
              ...
              {items.map(({ product, quantity }) => (
                <div key={product.id} className="summary-item">
                  ...
                  <p className="summary-item-price">${(product.price * quantity).toFixed(2)}</p>
                </div>
              ))}
